...
> **在体系中的位置**：第 6 步 · 边缘。先读 [架构总览与决策指南](overview.md)；位于 [微服务](microservices.md) 边缘，仅用官方 YARP（[POLICY P10](../governance/policy.md)）；聚合依赖各服务 [API 设计](api-design.md) 契约；写请求仍走命令，与 [事件驱动](event-driven.md) 分工。

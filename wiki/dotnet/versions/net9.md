...
.NET 9 于 2024-11 发布（**STS，支持到 2026-05**），对应 **C# 13**。本页汇总其关键能力；与 .NET 8 / .NET 10 的差异见 [.NET 版本演进](../../comparisons/net-evolution.md)。作为 STS，.NET 9 的全部特性均向下兼容到 .NET 10，并叠加更多新功能。

## C# 13 语言特性

- **params 集合**：`params ReadOnlySpan<T>` / `params IEnumerable<T>`，可接收任意集合 / 栈上 span，零分配友好。
- **新 `System.Threading.Lock`**：`Lock` 类型取代 `lock(obj)`，性能与可观测性更好。
- **params 与 ref 细化**、转译顺序与重载优先级改进。
...
- **内建 OpenAPI 生成器**：`Microsoft.AspNetCore.OpenApi` 的 `AddOpenApi()` 自 .NET 9 起可用，生成 **OpenAPI 3.0**（原生 OpenAPI 3.1 在 .NET 10 升级到 3.1，见 [原生 OpenAPI 3.1](../aspnet-core/aspnet-core-10.md#openapi-3-1)）。
- 静态资源缓存、计时器与各项目性能改进；原生 AOT 限制较 net8 减少。

## .NET 9 平台能力

- **Microsoft.Extensions.AI**：统一的 AI/LLM 抽象（`IChatClient`、`IEmbeddingGenerator`、工具调用），便于接入多种模型；这是 .NET 9 的标志性能力。

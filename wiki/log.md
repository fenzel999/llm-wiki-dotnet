...
- 2026-07-11 **架构区体系重设计（人类反馈：架构所有内容都要重新整理）**：在上一轮"体系化"基础上，把架构区从骨架到每页正文重做成一套连贯系统。
    - **导航重排为「六步决策旅程」**：`mkdocs.yml` 架构段改为 `架构（六步决策旅程）` → 第 1 步 边界形态（模块化单体/微服务）→ 第 2 步 模块内部结构（整洁/六边形/垂直切片/解决方案分层）→ 第 3 步 领域建模（DDD 战术构件）→ 第 4 步 跨边界契约（限界上下文/API 设计/ADR）→ 第 5 步 按需叠加能力（CQRS/事件溯源/事件驱动/Saga/多租户/审计软删除/架构测试）→ 第 6 步 边缘（API 网关 BFF）→ 导览与反模式。分组即教学顺序，消除"平铺罗列"。
    - **总览页改写为体系总线**：`architecture/overview.md` 重写成主干图（架构怎么长出来）+ 六步走逐页决策 + 决策速查表 + 推荐组合（默认栈/进阶跨进程栈）+ 由浅入深学习路径，所有 25 页在图中定位。
    - **每篇架构页正文纳入体系**：给 25 篇 `architecture/*.md` 顶部加「在体系中的位置」定位块（第几步·组名 + 先读总览 + 同组/常搭配链接），打开即自定位；配合各页已有的「何时使用 / 与其他模式的关系」，分散的页被串成节点。
    - **索引同步**：`index.md` 架构节改为六步分组，与新导航一致。
    - 修复本轮新增的相对路径（`wiki/architecture/` 内应为 `../governance` 与 `../patterns`、`../anti-patterns`）。
    - 影响：architecture/overview.md、全部 25 篇 architecture/*.md、mkdocs.yml、index.md、log.md。
    - 注：本轮为"骨架 + 每页定位 + 结构统一"的体系重设计；若你要进一步**重写各页技术正文/增删合并页面**，请指明，我再深入。

...
> **要点速览**
> - 命名、API 设计、空处理的规范合集，讲清"为什么"。
> - 命名清晰达意、遵循 .NET 约定；API 面向调用者设计。
> - 空值优先用可空引用类型 + 明确契约，减少 NRE。
...
注意，`MaxRetryCount` 与 `MAX_BATCH_SIZE` 只是不同团队的风格取舍（前者 PascalCase、后者全大写），关键是**在仓库内保持一致**，而不是某一种"绝对正确"的写法。下文谈到的"健壮与可演进"比命名细节更重要。
